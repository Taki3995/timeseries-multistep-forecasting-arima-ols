...
def adf_test_single(y, p, case=2):
    ...
    if case == 1:
        X = np.zeros((n_obs, 1 + p))
        X[:, 0] = y[p:n_obs+p]
        for i in range(1, p + 1):
            X[:, i] = dy[p - i : n_obs + p - i]
        gamma_idx = 0
    elif case == 2:
    ...
    se_gamma = np.sqrt(cov_matrix[gamma_idx, gamma_idx])
...
def adf_test_iterative(y, max_d=3, max_lag=5, alpha=0.05):
    ...
    for d in range(max_d + 1):
        ...
        best_p = 0
        best_case = 2
        is_stationary_d = False
        
        for case in [1, 2, 3]:
            for p in range(max_lag + 1):
                try:
                    adf_stat, sse, aic = adf_test_single(y_diff, p, case=case)
                    if aic < best_aic:
                        best_aic = aic
...
def main(data_path='tseries.csv', output_path='adf.csv', max_d=3, max_lag=5):
    ...
    for result in adf_results:
        case_name = {1: "Sin const", 2: "Drift", 3: "Tend+Drift"}[result['case']]
        print(f"d={result['d']} ({case_name}): τ̂={result['adf_stat']:.4f}, "
